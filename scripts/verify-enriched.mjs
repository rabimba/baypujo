...
// Prothoma — cultural programs section (bayarea)
if (BA) await check("/pujas/prothoma/", (t) => [
...
// Aantorik — hours + 6 programs + contact (bayarea)
if (BA) await check("/pujas/aantorik/", (t) => [
...
// Abahan — contact + zeffy + schedule (bayarea)
if (BA) await page.goto(`${BASE}/pujas/abahan/`, { waitUntil: "networkidle" });
if (BA) {
...
// Muktangan — Navrang + contacts + notice (bayarea)
if (BA) await check("/pujas/muktangan/", (t) => [
...
// Utsab — programs w/ ticketed (bayarea)
if (BA) await check("/pujas/utsab/", (t) => [
...
// Shirdi Sai — contact w/ WhatsApp note (bayarea)
if (BA) await check("/pujas/shirdisai/", (t) => [
...
const cityTag1 = CITY.plannerAddressExample.replace("e.g. ", "").split(",").pop().trim().split(" ")[0];
console.log(`${addr.length && addr[0].includes(cityTag1) ? "PASS" : "FAIL"}  planner — address geocode: ${addr[0] ?? "no status"}`);
...
const cityTag2 = CITY.plannerCityQuery.split(",")[0].trim();
console.log(`${addr2.length && addr2[0].includes(cityTag2) ? "PASS" : "FAIL"}  planner — city geocode: ${addr2[0] ?? "no status"}`);
